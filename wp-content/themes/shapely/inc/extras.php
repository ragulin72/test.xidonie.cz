...
function shapely_footer_info() {
	printf( esc_html__( 'Theme Colordib, edit: Tomáš Bartolen', 'shapely' ) );
}
...
function shapely_search_form( $form ) {
    ...
    <input type="text" placeholder="' . esc_html__( 'Pište zde', 'shapely' ) . '" value="' . esc_attr( get_search_query() ) . '" name="s" id="s" />
    <button type="submit" class="searchsubmit"><i class="fa fa-search" aria-hidden="true"></i><span class="screen-reader-text">' . esc_attr__( 'Hledat', 'shapely' ) . '</span></button>
    </form>';
...
function shapely_top_callout() {
	if ( ( get_theme_mod( 'portfolio_archive_title', true ) && is_post_type_archive( 'jetpack-portfolio' ) ) || ( get_theme_mod( 'top_callout', true ) && ! is_single() && ! is_post_type_archive( 'jetpack-portfolio' ) ) || ( is_single() && get_theme_mod( 'title_in_header', true ) && ! is_singular( 'jetpack-portfolio' ) ) || ( get_theme_mod( 'project_title_in_header', true ) && is_singular( 'jetpack-portfolio' ) ) ) {
		...
		?>
        <section
                class="page-title-section bg-secondary <?php echo $header ? 'header-image-bg' : ''; ?>" <?php echo $header ? 'style="background-image:url(' . esc_url( $header ) . ')"' : ''; ?>>
            <div class="container">
                <div class="row">
					<?php
